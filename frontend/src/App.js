  ...
  const items = [
    {
      ...
      icon: <List size={18} />,
      content:
        'Automate the extraction of P&L, Balance Sheet, and Cash Flow statements from complex multi-page financial PDFs using AI Vision.'
    },
    {
      title: 'Project Limitations',
      icon: <AlertTriangle size={18} />,
      content:
        'Maximum file size is 20MB. AI parsing credits are limited. Accuracy depends on the scan quality.'
    },
    {
      title: 'Described Answer',
      icon: <FileText size={18} />,
      content:
        'Frontend:-React.js,Tailwind CSS,Flowbite | Backend:-Python, FastAPI, Uvicorn,python-dotenv,Gemini 2.5 Flash, LlamaParse,Camelot '
    },
    {
      title: 'GitHub Link',
      icon: <Github size={18} />,
      content: (
        <a
          href="https://github.com/kmarankit/Money-Stories-Final/"
          target="_blank"
          rel="noreferrer"
...
const MessageModal = ({ isOpen, title, message, onClose }) => {
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      ...
      >
        <button className="modal-close-btn" onClick={onClose} aria-label="Close message">
          <X size={18} />
        </button>
        <div className="modal-badge">
          <CircleAlert size={20} />
        </div>
        <h4 id="message-modal-title">{title}</h4>
        <p>{message}</p>
  ...
  const handleFileChange = async (e) => {
    ...
    fileReader.onload = async function () {
      try {
        ...
        if (totalPages > 15) {
          showModal('Upload Limit Reached', 'PDF exceeds 15 pages. Please upload a shorter file.');
          return;
        }
        ...
        showModal('Invalid File', 'This PDF could not be validated. Please upload a valid PDF file.');
      }
    };
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        const extracted = response.data.financialData;
      ...
      setData({ pnl: [], balanceSheet: [], cashFlow: [], others: [] });
      setExcelBuffer(null);
      showModal(
        'No Financial Data Found',
        response.data.message || 'The uploaded PDF does not contain extractable financial statements.'
      );
    } catch (err) {
      ...
      showModal(
        'Extraction Failed',
        backendDetail || backendMessage || 'Extraction failed. Please try again with a valid financial PDF.'
      );
    } finally {
                ...
                <section className="result-card">
                  ...
                  <div className="table-wrapper">
                    <table className="data-table">
                      <thead>
                        <tr>{Object.keys(data.pnl[0]).map((k) => <th key={k}>{k.replace(/_/g, ' ')}</th>)}</tr>
                      </thead>
                      <tbody>
                        {data.pnl.map((row, i) => (
                          <tr key={i}>
